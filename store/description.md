Interval is a spaced-repetition flashcard app that decides what to show you and when, based on how well you actually remember it — so you spend review time on the things you're about to forget, not the things you already know cold.

Whether you're learning a language, studying for an exam, or memorizing anything else, Interval builds a personal review schedule from your own performance using FSRS, a modern memory-modeling algorithm, and adjusts it every time you grade a card.

EVERYTHING STAYS ON YOUR DEVICE
No account, no sign-in, no cloud sync required. Your decks, cards, and study history live in a local database on your phone — nothing is uploaded anywhere. Automatic local backups run in the background, and you can export or restore a full backup file yourself anytime from Settings. Already have an Anki collection? Import your existing .apkg decks and keep studying right where you left off.

BUILT FOR HOW YOU ACTUALLY STUDY
• Gesture-based review — swipe to grade, tap to reveal, with buttons always available too
• Cloze deletions ({{c1::hidden text}}) with an in-app guide if you're new to the syntax
• Rich text formatting: bold, italic, underline, superscript, subscript
• Embed photos from your camera or library, or record audio right on the card — pronunciation, a voice note, whatever helps it stick
• Audio plays automatically when you flip the card, with a one-tap replay
• Image Occlusion — photograph a diagram, draw boxes over the parts to test yourself on, and Interval turns each box into its own card
• Custom note types and templates if the built-in Basic, Cloze, and type-in-answer formats aren't enough

STAY CONSISTENT
• Day streaks with a freeze mechanic so one missed day doesn't erase your progress
• An optional daily reminder at whatever time works for you
• A statistics dashboard showing your review history, retention rate, and where your cards stand

MAKE IT YOURS
• Light and dark themes with a choice of accent colors
• Adjustable card font size and family
• A reduced-motion setting for a calmer interface
• 2, 3, or 4 answer buttons depending on how much nuance you want when grading

POWERFUL BROWSING
Search and filter your entire collection with Anki-style queries, manage tags, bulk-edit cards, and recover anything you've deleted from a built-in trash before it's gone for good.

Interval is free to use, with no ads and no in-app purchases required to study. If you find it useful, there's an optional way to support development in Settings — entirely up to you.
